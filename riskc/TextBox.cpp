...
//needs a big cleanup tho
	...
	TextBox::TextBox(int x, int y, int height, int width, std::string startingString) {
		font = new sf::Font();
	...
	void TextBox::calculateCharBounds(sf::String s) {
		text.setString(s);
	...
	void TextBox::moveCursor(sf::Vector2i mousePos, sf::Event event) {
	...
	void TextBox::collide(sf::Vector2i mousePos, sf::RenderWindow& window, sf::Cursor& cursor) {
		if (box.getGlobalBounds().contains(mousePos.x, mousePos.y)) {
